...
INIT = Pig.compile("""
A = LOAD 'gs://lddm_bucket/page_links_en.nt.bz2' using PigStorage(' ') as (url:chararray, p:chararray, link:chararray);
B = GROUP A by url;                                                                                  
...
params = { 'd': '0.5', 'docs_in': 'gs://lddm_bucket/out/pagerank_data_simple' }
...
for i in range(3):
   out = "gs://lddm_bucket/out/pagerank_data_" + str(i + 1)
   params["docs_out"] = out
   Pig.fs("rmr " + out)
